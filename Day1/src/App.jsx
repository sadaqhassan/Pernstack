...
const handleDelete = (id)=>{
  ...
  return (
    <div className='flex flex-col justify-center items-center '>
      ...
      <button className='bg-cyan-600 px-2 py-1 rounded-xl text-white'>Add new +</button>
      </div>
    ...
    
      {
        ...
        tasks.map((task,index)=>(
          ...
          <button className='bg-green-600 text-white text-sm px-2 py-1 rounded ' onClick={()=>{setOpenEdit(true) ; setUpdateTask(task)}}>Edit</button>
      ...
      <div className='flex flex-col  p-10 bg-gray-300 rounded-2xl' onClick={(e)=>e.stopPropagation()}>
        ...
        <button onClick={handleSubmit} className='bg-green-600 text-white text-sm px-2 py-1 rounded mt-5'>Edit</button>
      </div>
      </div>
      }
    </div>
